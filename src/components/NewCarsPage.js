...
export default function NewCarPage() {
  ...
  let getFiltersFromUrl = () => {
    let filterUrl = window.location.href.split('/new-cars')[1].replace('?', '')
  ...
  const fetchCars = async () => {
    try {
      const response = await api.get('/vehicles/?condition=New&limit=10&' + window.location.href.split('/new-cars?')[1])
